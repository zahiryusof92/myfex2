...
class Amendment extends Model {
    ...
    public static function getApproved() {
        if (Auth::user()->isUser()) {
            $approved = Amendment::where('company_id', Auth::user()->company_id)->where('status', SELF::DILULUS)->count();
        } else if (Auth::user()->isConsultant()) {
            $approved = Amendment::where('consultant_id', Auth::user()->id)->where('status', SELF::DILULUS)->count();
        }
    ...
    public static function getTotalPending() {
        ...
        if (Auth::user()->isUser()) {
            $total = Amendment::where('company_id', Auth::user()->company_id)->where('status', SELF::DRAF)->count();
        } else if (Auth::user()->isConsultant()) {
            $total = Amendment::where('consultant_id', Auth::user()->id)->where('status', SELF::DRAF)->count();
        } else if (Auth::user()->isPPU()) {
            $total = Amendment::where('status', SELF::BARU)->count();
        } else if (Auth::user()->isKPP()) {
            $total = Amendment::where('status', SELF::DISYOR)->count();
        } else if (Auth::user()->isPengarah()) {
            $total = Amendment::where('status', SELF::DISOKONG)->count();
        } else if (Auth::user()->isPendaftar()) {
            $total = Amendment::where('status', SELF::DIPERAKUI)->count();
        }
